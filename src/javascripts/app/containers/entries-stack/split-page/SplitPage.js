...
const EntrySplitPage = ({ entry, fetching, onSplitEntry, timezone }) => {
  // componentDidMount() {
  ...
  const _getInitialValuesAndHours = (entry) => {
    if (entry) {
  ...
  const initialValues = _getInitialValuesAndHours(entry);
